...
window.addEventListener("load", ()=>{ //setup the checksum validator
    (function() {
        ...
        UploaderModel.UploadItem.prototype.uploadPresigned = function() {
            ...
            workerManager.generateChecksum(this._file)
                .then(checksumData => {
        ...
        function validateChecksum(node, expectedChecksum, filePath, retryCount) {
            ...
            if (node.Etag === "temporary" && retryCount < maxRetries) {
                ...
                console.error('Checksum validation failed.', 'Expected:', expectedChecksum, 'Received:', etag);
