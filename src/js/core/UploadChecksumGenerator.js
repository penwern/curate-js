...
function searchNodesWithTokens(bearerToken, body) {
    // Return the promise chain
    return PydioApi._PydioRestClient.getOrUpdateJwt().then(token => {
...
function generateJson(files) {  //convert upload input to nodepaths
    const ps = Object.values(files).map(obj => `${Curate.workspaces.getOpenWorkspace()}/${obj.webkitRelativePath}`);
    const result = {
        "NodePaths": ps
    }
        ...
        .then(userId => {
            ...
            if (type == "userfile") {
                ...
                const searchParent = searchNodesWithTokens(token, JSON.stringify({  //search Curate for uploaded nodes
                    "NodePaths": [`${Curate.workspaces.getOpenWorkspace()}/${f[0].webkitRelativePath.split("/")[0]}`]
                }))
                searchParent
                    .then(r => r.json())
                ...
                .then(scriptContent => {
                    if (scriptContent) {
                            ...
                            authSession.then((token) => {
                                    const basePath = Curate.workspaces.getOpenWorkspace() + (pydio._dataModel._currentRep !== "/" ? pydio._dataModel._currentRep + "/" : "");
                        ...
                        console.error('Empty script content received.');
                    }
                })
                .catch(error => {
...
async function uploadFiles(files) {
    try {
        ...
        results.forEach((result, index) => {
            console.log(`Upload for file ${files[index].name} completed with result: `, result);
        });
...
function getFullPath(name) {
    return Curate.workspaces.getOpenWorkspace() + "/" + name;
}
...
document.addEventListener("drop", async function (e) {
    if (e.dataTransfer && e.target.className !== "drop-zone dropzone-hover") {
        ...
        let files = [...dt.files]; 
        var checksums = await processItems(dt);
        let taskCounter = files.length;

        files.forEach(file => {
            pydio.observeOnce("longtask_finished", () => {
                taskCounter--;
                if (taskCounter === 0) {
                    // Perform actions when all tasks for all files are finished
                    setTimeout(()=>{
